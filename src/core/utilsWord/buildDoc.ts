...
export async function createNovedadesDoc(input: BuildDocInput): Promise<Document> {
  ...
  for (let i = 0; i < input.novedades.length; i++) {
    ...
    if (!k) continue;
    if (!sectores.has(k)) sectores.set(k, { label: (n.sectorGeneral || '').trim(), items: [] });
  ...
  for (let s = 0; s < sectorKeys.length; s++) {
    ...
    for (const n of sectorGroup.items) {
      ...
      if (!ak) continue;
      if (!areas.has(ak)) areas.set(ak, { label: (n.areaNovedad || '').trim(), items: [] });
    ...
    for (let a = 0; a < areaKeys.length; a++) {
      ...
      // 3) Ordenar novedades
      const ordenadas = areaGroup.items.slice().sort((x, y) => cmpBase(x.tituloNovedad, y.tituloNovedad));
      ...
      for (let k = 0; k < ordenadas.length; k++) {
        ...
        if (typeof resumenHtml === 'string' && resumenHtml.trim()) {
          out.push(...htmlToParagraphsControlled(resumenHtml));
        }
  ...
  return new Document({
    ...
    sections: [
      {
        headers: { default: buildHeader(input.confidentialityLabel ?? '') },
        footers: { default: buildFooter(input.confidentialityLabel ?? '') },
        children: out,
      },
    ],
  });
